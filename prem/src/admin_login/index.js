...
class Login extends React.Component {
  ...
  componentDidMount() {
    ...
    if (
      window.sessionStorage.getItem(Constants.ADMIN_ACCESS_TOKEN) !==
        undefined &&
      window.sessionStorage.getItem(Constants.ADMIN_ACCESS_TOKEN) != null
    ) {
      console.log(
        "Clear expired keys..." +
          window.sessionStorage.getItem(Constants.ADMIN_ACCESS_TOKEN)
    ...
    if (this.props.isLoginSuccess !== prevProps.isLoginSuccess) {
      if (this.state.submitted && this.props.isLoginSuccess.success) {
        window.sessionStorage.setItem(
          Constants.ADMIN_ACCESS_TOKEN,
          this.props.isLoginSuccess.accessToken
        );
        this.setState({ isLoading: false, showOtpModal: true });
  ...
  render() {
    if (this.state.isReadyToRedirect) return <Redirect to="/dashBoard" />;
          ...
          >
            <div className="header-container">
              <ul className="nav-left">
                <li>
                <img src={Logo} alt="CtrlSwift" className="admin_logo_width"  />
                </li>
              </ul>
            </div>
          </div>
                  ...
                  <div className="row ">
                    <div className="col-md-4 p-40">
                      <div className=" card bgc-white p-40 bd logincontent">
                        <h5 className="text-center font-weight-bold"> Admin Login</h5>
                        <div className="mT-30">
                          <form name="form" onSubmit={this.handleSubmit}>
                            ...
                            <br />
                            <br />
                            <Link to="/forgotpassword?page=admin" id="cs-admin-fg">
                              Forgot My Password
                            </Link>
                          </form>
                        </div>
          ...
          >
            ...
            <Modal.Body>
              <div className="form-group">
                ...
                />
                {otpSubmitted && !otp && (
                  <div style={{ fontSize: 12, color: "red" }}> OTP requires</div>
                )}
                { otpSubmitted && otp.length !==6 && otp &&(
                  <div style={{ fontSize: 12, color: "red" }}> OTP is must be 6-digits</div>
                )}
                ...
                <div className="row">
                  <div className="col-md-5">
                    <button
                      ...
                      className="buttons btn btn-primary btn-color-admin"
                      style={{ width: "100%" }}
                      id="cs-admin-otp-btn"
                    >
                      SUBMIT
...
const mapStateToProps = (state) => {
  return {
    isLoginSuccess: state.adminloginreducer.isLoginSuccess,
